...
describe('buying over the game socket', () => {
  it.each([false, true])('fills at the server price with open market=%s and preserves the first answer', async (open) => {
    ...
    try {
      ...
      client.socket.ping(); await pong;
      h.sample(); const quoted = await client.nextFrame();
      expect(quoted.draft).toMatchObject({ contractId, spendCents: 100000 });
      const command = { t: 'buy', commandId: 'buy-once', day: 1, contractId, spendCents: 100000, seenPriceCents: 1000000 };
      client.send(command); const result = await client.next();
      expect(result.t).toBe('reply');
      if (result.t !== 'reply') throw new Error('buy did not receive an outcome');
      expect(result.receipt).toMatchObject({ kind: 'buy', commandId: command.commandId, outcome: 'accepted' });
      expect(result.frame.positions).toHaveLength(1);
      expect(result.frame.positions[0]).toMatchObject({ entryPriceCents: quoted.draft!.ticket!.priceCents,
        quantity: quoted.draft!.ticket!.quantity, costCents: quoted.draft!.ticket!.costCents });
      expect(result.frame.account.cashCents).toBeLessThan(lobby.account.cashCents);
      ...
      expect(late.frame.positions[0]?.status).toBe('settled');
      h.sample(); expect((await client.nextFrame()).account).toEqual(late.frame.account);
  ...
  it('leaves cash and the daily slot intact after rejection', async () => {
    const h = await startHarness();
    try {
      const client = h.connect(); await client.opened(); client.send({ t: 'hello', v: PROTOCOL_VERSION }); await client.nextFrame();
      client.send({ t: 'start', pace: 1, commandId: 'start-game' }); const before = (await client.nextReply()).frame;
      client.send({ t: 'buy', commandId: 'bad-contract', day: 1, contractId: 999999, spendCents: 100000, seenPriceCents: 1000 });
      const answer = await client.next(); expect(answer.t).toBe('reply');
      if (answer.t !== 'reply') throw new Error('missing rejection');
      expect(answer.receipt).toMatchObject({ outcome: 'rejected', reason: 'unknownContract' });
      expect(answer.frame.account.cashCents).toBe(before.account.cashCents);
