...
import { FIRST_PLAYER_ID, PROTOCOL_VERSION, isOffered, parseServerMessage, playerOf, seedToMarketCode } from '@strike-desk/shared/engine';
import type { BuyCommand, Frame, Pace } from '@strike-desk/shared/engine';
import { handle } from '../src/modules/command-path/index';
import { frameAt, msAtStep, quotedAt, sessionAt, tradableTicket } from './contracts/sessionAt';
...
describe('authoritative buy boundaries', () => {
  it.each(['notOffered', 'tooCheap'] as const)('checks %s before invalid cash and tolerance', (reason) => {
    const fixture = sessionAt('open');
    const session = reason === 'notOffered'
      ? { ...fixture.session, market: { ...fixture.session.market, offeredPassedMoves: 0.4 } }
      : fixture.session;
    const nowMs = msAtStep(799);
    const frame = frameAt(session, nowMs);
    const contractId = frame.quotes.findIndex((price, id) => reason === 'notOffered'
      ? !isOffered(frame.board!, id) : isOffered(frame.board!, id) && price < frame.minTicketCents);
    expect(contractId).toBeGreaterThanOrEqual(0);
    const result = handle({ session, playerId: FIRST_PLAYER_ID, nowMs, draft: null,
      command: { t: 'buy', commandId: 'invalid-ticket', day: 1, contractId, spendCents: 100000001, seenPriceCents: 1 } });
    expect(result.reply.receipt).toMatchObject({ outcome: 'rejected', reason });
    expect(result.reply.frame.account.cashCents).toBe(100000000);
  });

  it.each([true, false])('settles a held ticket with a positive payout=%s only once across late reads and retries', (pays) => {
    const { session, nowMs } = sessionAt('beforeBell');
    const frame = frameAt(session, nowMs);
    const candidates = frame.quotes.flatMap((price, contractId) => {
      if (price < frame.minTicketCents || !isOffered(frame.board!, contractId)) return [];
      const command: BuyCommand = { t: 'buy', commandId: 'held-ticket', day: 1, contractId,
        spendCents: 10000000, seenPriceCents: price };
      const bought = handle({ session, playerId: FIRST_PLAYER_ID, nowMs, draft: null, command });
      const settled = frameAt(bought.session, msAtStep(800));
      const payout = settled.positions[0]?.exit?.proceedsCents;
      return bought.reply.receipt.outcome === 'accepted' && payout !== undefined && (payout > 0) === pays
    ...
    expect(settled.account.cashCents).toBe(bought.reply.frame.account.cashCents + payout);
    let current = bought.session;
    for (const step of [800, 801, 950, 1800]) {
      const repeated = handle({ session: current, playerId: FIRST_PLAYER_ID, nowMs: msAtStep(step), draft: null, command });
      expect(repeated.reply.receipt).toEqual(bought.reply.receipt);
      expect(repeated.reply.frame.account.cashCents).toBe(settled.account.cashCents);
      expect(repeated.reply.frame.positions[0]?.exit).toEqual(settled.positions[0]?.exit);
      expect(playerOf(repeated.session.game, FIRST_PLAYER_ID).log).toHaveLength(2);
  ...
  it('uses the server fill and logs a refused request only once before a fresh successful request', () => {
    ...
    const command: BuyCommand = { t: 'buy', commandId: 'buy-too-small', day: 1,
      contractId: ticket.contractId, spendCents: 1, seenPriceCents: 10000 };
    const refused = handle({ session, playerId: FIRST_PLAYER_ID, command, nowMs, draft: null });
    expect(refused.reply.receipt).toMatchObject({ outcome: 'rejected', reason: 'spendTooSmall' });
    expect(playerOf(refused.session.game, FIRST_PLAYER_ID)).toMatchObject({ cashCents: 100000000, rev: 2, positions: [] });
    expect(playerOf(refused.session.game, FIRST_PLAYER_ID).log).toHaveLength(2);
    const repeat = handle({ session: refused.session, playerId: FIRST_PLAYER_ID,
      command: { ...command, spendCents: 2500 }, nowMs, draft: null });
    expect(repeat.reply.receipt).toEqual(refused.reply.receipt);
    expect(repeat.session).toBe(refused.session);
    const bought = handle({ session: repeat.session, playerId: FIRST_PLAYER_ID,
      command: { ...command, commandId: 'buy-fresh', spendCents: 2500 }, nowMs, draft: null });
    expect(bought.reply.receipt.outcome).toBe('accepted');
    expect(bought.reply.frame.positions[0]).toMatchObject({ entryPriceCents: 1000, quantity: 2, costCents: 2000 });
    expect(bought.reply.frame.account).toMatchObject({ cashCents: 99998000, canBuy: false });
    expect(playerOf(bought.session.game, FIRST_PLAYER_ID).log).toHaveLength(3);
  });
  ...
  ] as const)('preserves refusal priority at %s, day %s, stress %s, contract %s, spend %s, seen %s',
    (moment, day, stress, contract, spendCents, seenPriceCents, reason) => {
      const fixture = sessionAt(moment === 'final' ? 'open' : moment, stress ? { targetsPerCompany: 209 } : {});
      const nowMs = moment === 'final' ? msAtStep(4500) : fixture.nowMs;
      const contractId = contract ?? quotedAt(frameAt(fixture.session, nowMs), 1000).contractId;
      const result = handle({ session: fixture.session, playerId: FIRST_PLAYER_ID, nowMs, draft: null,
        command: { t: 'buy', commandId: 'priority-buy', day, contractId, spendCents, seenPriceCents } });
      expect(result.reply.receipt).toMatchObject({ outcome: 'rejected', reason });
      expect(result.reply.frame.positions).toEqual([]);
      expect(result.reply.frame.account.cashCents).toBe(100000000);
      expect(playerOf(result.session.game, FIRST_PLAYER_ID).log.at(-1)?.command.commandId).toBe('priority-buy');
    });
  ...
  it('checks an existing daily ticket before an unknown contract, including a settled ticket', () => {
    ...
    const ticket = quotedAt(frameAt(session, nowMs), 1000);
    const bought = handle({ session, playerId: FIRST_PLAYER_ID, nowMs, draft: null,
      command: { t: 'buy', commandId: 'first-ticket', day: 1, contractId: ticket.contractId, spendCents: 2500, seenPriceCents: 1000 } });
    const closed = handle({ session: bought.session, playerId: FIRST_PLAYER_ID, nowMs, draft: null,
      command: { t: 'cashOut', commandId: 'settled-ticket', positionId: 'd1' } });
    for (const current of [bought.session, closed.session]) {
      const result = handle({ session: current, playerId: FIRST_PLAYER_ID, nowMs, draft: null,
        command: { t: 'buy', commandId: 'another-ticket', day: 1, contractId: 999999, spendCents: 100000001, seenPriceCents: 1 } });
      expect(result.reply.receipt).toMatchObject({ outcome: 'rejected', reason: 'alreadyBought' });
      expect(result.reply.frame.positions).toHaveLength(1);
    }
  });

  it.each([799, 800, 801])('judges an actual socket buy at logical step %s', async (step) => {
    const h = await startHarness();
    try {
      const client = h.connect(); await client.opened(); client.send(hello); await client.nextFrame();
      client.send({ t: 'start', pace: 1, commandId: 'bell-start' }); await client.nextReply();
      h.clock.advance(step * 200); h.sample(); const before = await client.nextFrame();
      const ticket = tradableTicket(before);
      client.send({ t: 'buy', commandId: 'bell-buy', day: 1, contractId: ticket.contractId,
        spendCents: 10000000, seenPriceCents: ticket.priceCents });
      const result = await client.nextReply();
      expect(result.receipt).toMatchObject(step === 799 ? { outcome: 'accepted', step: 799 }
        : { outcome: 'rejected', reason: 'marketClosed', step });
      if (step === 799) {
        expect(result.frame.positions).toHaveLength(1);
        h.clock.advance(200); h.sample(); const settled = await client.nextFrame();
        expect(settled.positions[0]).toMatchObject({ status: 'settled', exit: { kind: 'bell' } });
        h.sample(); expect((await client.nextFrame()).account).toEqual(settled.account);
      } else {
  ...
  it('charges a command shared by two real sockets only once', async () => {
    const h = await startHarness();
    try {
      const first = h.connect(); await first.opened(); first.send(hello); const lobby = await first.nextFrame();
      first.send({ t: 'start', pace: 1, commandId: 'two-tabs-start' }); const frame = (await first.nextReply()).frame;
      const second = h.connect(); await second.opened(); second.send({ ...hello, session: lobby.session }); await second.nextFrame();
      const ticket = tradableTicket(frame);
      const command = { t: 'buy', commandId: 'shared-buy', day: 1, contractId: ticket.contractId,
        spendCents: 100000, seenPriceCents: ticket.priceCents };
      first.send(command); second.send(command);
      const [one, two] = await Promise.all([first.nextReply(), second.nextReply()]);
  ...
  it('keeps exhausted door requests anonymous and outside cash, positions, receipts and input log', () => {
    const registry = createRegistry({ drawSeed: () => FIXED_SEEDS[0]!, drawId: () => 'limited-game', limits: LIMITS });
    const sent: string[] = [];
    const connection: Connection = { sessionId: null, playerId: null, messages: createWindowCounter(2, 10000),
      socket: { OPEN: 1, readyState: 1, bufferedAmount: 0, send: (text) => { sent.push(text); } } };
    const options = { registry, now: () => 0, maxBoardSize: PUBLIC_MAX_BOARD_SIZE, newSessions: createTokenBucket(30, 3) };
    for (const message of [hello, { t: 'start', pace: 1, commandId: 'limit-start' }]) handleInbound(options, connection, JSON.stringify(message));
    ...
    const contractId = startReply.frame.quotes.findIndex((price, id) => price >= startReply.frame.minTicketCents && isOffered(startReply.frame.board!, id));
    handleInbound(options, connection, JSON.stringify({ t: 'buy', commandId: 'limited-buy', day: 1, contractId, spendCents: 100000, seenPriceCents: 1000000 }));
    expect(JSON.parse(sent.at(-1)!)).toEqual({ t: 'error', code: 'tooManyCommands' });
    expect(registry.get('limited-game')!.session).toBe(before);
    expect(playerOf(before.game, FIRST_PLAYER_ID)).toMatchObject({ cashCents: 100000000, positions: [], rev: 1 });
