...
describe('resume: a hello that names a running game', () => {
  ...
  it('receives the original command outcome on resume, samples and replies without another command revision', () => {
    // A lost answer is recoverable from the next whole frame. An explicit
    // retry remains safe and returns the very same stored outcome.
    const { made, first } = runningGame();
    const returning = made.connect();
    say(made, returning, { ...HELLO, session: 'session-a' });

    const receipt = { commandId: 'start-0001', kind: 'start', step: 0, outcome: 'accepted' };
    expect(lastFrame(returning).receipts).toEqual([receipt]);
    expect(lastFrame(first).receipts).toEqual([receipt]);

    const reply = JSON.parse(first.sent[1] ?? 'null') as { t: string; receipt: { commandId: string }; frame: unknown };
    expect(reply.t).toBe('reply');
    expect(reply.receipt).toEqual(receipt);
    expect(frameSchema.parse(reply.frame).receipts).toEqual([receipt]);
    ...
    expect(frameSchema.parse(again.frame).receipts).toEqual([receipt]);
  });
