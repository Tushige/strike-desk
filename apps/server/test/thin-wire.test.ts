...
const wholeCentsFromZero = (values: readonly number[]): boolean =>
    ...
    return {
        ...
        boardSize:
            frame.board === null
                ? null
                : {
                      targetsPerCompany: frame.board.targetsPerCompany,
                      targetsOfEachCompany: frame.board.companies.map(
                          (company) => company.targets.length,
                      ),
                  },
        ...
        everyQuoteIsWholeCentsFromZero:
            wholeCentsFromZero(frame.quotes) &&
            wholeCentsFromZero(frame.quoteReals) &&
            wholeCentsFromZero(frame.quoteHopes) &&
            wholeCentsFromZero(frame.quoteBreakEvens),
        quotesThatAreNotRealPlusHope: frame.quotes.flatMap((price, id) =>
            (frame.quoteReals[id] ?? Number.NaN) +
                (frame.quoteHopes[id] ?? Number.NaN) ===
            price
                ? []
                : [id],
        ),
        companyKeys: frame.companies.map((company) =>
            Object.keys(company).sort().join(","),
        ),
        newsCount: frame.news.length,
        positions: frame.positions,
        receipts: frame.receipts.map(({ kind, step, outcome }) => ({
            kind,
            step,
            outcome,
        })),
        ...
        everyPriceIsWholeCents: frame.prices.every((price) =>
...
function thinAt(where: string): Record<string, unknown> {
    ...
    const finishedDays: Record<string, number[]> = {
        "the closing bell of day 1": [1],
        "the last step of day 1": [1],
        "the first step of day 2": [1],
        "day 5's debrief": [1, 2, 3, 4, 5],
        "the last step of the game": [1, 2, 3, 4, 5],
    ...
    return {
        ...
        boardSize: started
            ? {
                  ...
                  targetsOfEachCompany: Array.from(
                      { length: COMPANIES },
                      () => TARGETS_PER_COMPANY,
                  ),
              }
        ...
        positions: [],
        receipts: started
            ? [{ kind: "start", step: 0, outcome: "accepted" }]
            : [],
        days: (finishedDays[where] ?? []).map((day) => ({
            day,
        ...
        canBuy: [
            "the first step",
            "the last step before the opening bell",
            "the first open step",
            "the middle of day 1",
            "the first step of day 2",
            "draft sample",
        ].includes(where),
        stress: false,
        hasHistory: [
            "the first open step",
            "the middle of day 1",
            "the closing bell of day 1",
            "the first step of day 2",
            "day 5's debrief",
            "one step past the end of the game",
            "draft sample",
        ].includes(where),
...
describe("every frame this service emits", () => {
    it.each([undefined, 209])(
        "keeps serialized history secret at every reveal and day boundary (targets %s)",
        (targets) => {
            const original = buildMarket({
                seed: SEED,
                engine: ENGINE_VERSION,
                content: CONTENT_VERSION,
            });
            const firstReveal = Math.min(
                ...original.days[0]!.news.map(
                    (item) => item.hidden.revealIndex,
                ),
            );
            function emitted(market: Market, step: number): string {
                const registry = createRegistry({
                    drawSeed: () => SEED,
                    drawId: () => "serialized-history",
                    limits: LIMITS,
                });
                const entry = registry.create(0, targets)!;
                registry.replace(entry.session.id, {
                    ...handleCommand(
                        entry.session,
                        FIRST_PLAYER_ID,
                        { t: "start", commandId: "history-secrecy", pace: 1 },
                        0,
                    ).session,
                    market,
                });
            ...
            for (const step of [
                ...
                for (const day of changed.days) {
                    ...
                    day.paths = day.paths.map((path) =>
                        path.map((price, index) =>
                            today && index <= moment.priceIndex
                                ? price
                                : price * 1.37 + 11 + index,
                        ),
                    );
                    if (!today)
                        day.leadIn = day.leadIn.map((path) =>
                            path.map((price) => price + 17),
                        );
                    for (const item of day.news) {
                        if (
                            today &&
                            item.hidden.revealIndex <= moment.priceIndex
                        )
                            continue;
                        const next = today ? moment.priceIndex + 1 : 1;
                        item.hidden.revealIndex =
                            item.hidden.revealIndex === next ? next + 1 : next;
    ...
    it("was collected at every moment of the game", () => {
        expect(collected.map(({ where }) => where)).toEqual([
            LOBBY,
            ...MOMENTS.map(([where]) => where),
        ]);
    });
    ...
    it("fills public quotes and ordered game results with buying available only during active days", () => {
        for (const sampled of collected) {
            ...
            expect({
                where: sampled.where,
                account: sampled.frame.account,
            }).toEqual({
                where: sampled.where,
                // $1,000,000 × 100 cents; the cap is half, $500,000 × 100.
                account: {
                    cashCents: STARTING_CASH_CENTS,
                    worthCents: STARTING_CASH_CENTS,
                    capCents: 50_000_000,
                    canBuy: thinAt(sampled.where)["canBuy"],
                },
            });
        }
    });
    ...
    it("carries three current headlines with only public fields even after the bell and at final", () => {
        for (const { frame } of collected) {
            ...
            expect(frame.news.map((news) => news.trust).sort()).toEqual([
                1, 2, 3,
            ]);
            expect(new Set(frame.news.map((news) => news.companyId)).size).toBe(
                3,
            );
    ...
    it("round-trips the shared schema unchanged", () => {
        for (const { where, frame } of collected) {
            expect({ where, frame: frameSchema.parse(frame) }).toEqual({
                where,
                frame,
            });
            expect(frame.leadIn !== undefined).toBe(
                frame.history !== undefined,
            );
    ...
    it("carries neither the market code, nor the seed, nor even the name of the field it would live in", () => {
        ...
        expect(code).toMatch(
            /^[2-9A-HJ-NP-Z]{3}-[2-9A-HJ-NP-Z]{3}-[2-9A-HJ-NP-Z]{4}$/,
        );
        expect(codeWithoutDashes).toHaveLength(10);

        for (const { where, frame } of collected) {
            if (frame.clock.phase === "final") {
                expect(frame.final).toMatchObject({
                    marketCode: code,
    ...
    it("never lets the step go back, across a day boundary or at the end of the game", () => {
        ...
        expect(
            collected.map(
                ({ frame }) =>
                    `${frame.clock.phase} ${String(frame.clock.day)}`,
            ),
        ).toEqual([
            "lobby 0",
            "preBell 1",
...
describe("every batch of changed quotes this service emits", () => {
    ...
    function scrambleFrom(
        ...
        for (const marketDay of copy.days) {
            ...
            marketDay.paths = marketDay.paths.map((path) =>
                path.map((price, index) =>
                    index < from ? price : price * 1.37 + 11 + index,
                ),
            );
        }
        return copy;
    ...
    function batchOn(market: Market | null): QuotesMessage {
        ...
        registry.replace(
            entry.session.id,
            market === null ? started.session : { ...started.session, market },
        );
        ...
        sampleSessions(
            registry,
            (BATCH_STEP - 1) * STEP_MS,
            stats,
            LIMITS.stressFullFrameMs,
        );
        sampleSessions(
            registry,
            BATCH_STEP * STEP_MS,
        ...
        if (whole === undefined || batch === undefined)
            throw new Error(
                "the session did not send a whole picture and then a batch",
            );
        const parsed = parseServerMessage(JSON.parse(batch));
        if (parsed === null || parsed.t !== "quotes")
            throw new Error("the second message was not a batch");
    ...
    it("carries neither the market code, nor the seed, nor even the name of the field it would live in", async () => {
        const running = await startHarness({
            limits: { stressFullFrameMs: 1_000_000 },
        });
        try {
            ...
            for (const step of [BATCH_STEP, BATCH_STEP + 1, BATCH_STEP + 2]) {
                running.clock.advance(
                    startedAtMs + step * STEP_MS - running.clock.now(),
                );
                running.sample();
            }
            ...
            for (const raw of client.received()) {
                ...
                if (message !== null && message.t === "quotes")
...
describe("cash-out refusals on the ordinary wire", () => {
    ...
    /**
     * A well-formed cash-out with nothing to sell is answered by the rules,
     * with a rejected receipt. The receipt is kept, so the revision moves; the
     * cash does not.
     */
    const cashOut: CashOutCommand = {
        t: "cashOut",
        commandId: "nothing-to-sell",
        positionId: "d1",
    };
    const REJECTED = [cashOut];

    it.each(REJECTED)(
        "$t with no ticket is rejected by the rules, and the cash is untouched",
        async (command) => {
...
function expectDraftAtFrame(
    ...
    expect(frame.draft?.ticket?.whatIf).toContainEqual({
        atCents: frame.quoteBreakEvens[contractId],
        profitCents: 0,
    });
    expect(frame).toMatchObject({
        positions: [],
        days: [],
        account: { cashCents: STARTING_CASH_CENTS, canBuy: !frame.stress },
    });
...
describe("connection-local draft quotes", () => {
    it("answers only in the next sample at that frame price without a receipt or cash change", async () => {
        ...
        try {
            ...
            expect(allowList({ where: "draft sample", frame })).toEqual({
                ...thinAt("draft sample"),
                hasDraft: true,
            });
            await roundTrip(client);
    ...
    it.each([undefined, 2500])(
        "isolates two latest drafts and a draftless socket at the existing cadence (board %s)",
        async (board) => {
            const running = await startHarness();
            try {
                ...
                for (const client of [second, plain]) {
                    ...
                    expect(await client.nextFrame()).not.toHaveProperty(
                        "draft",
                    );
                }
                first.send({
                ...
                const [a, b, c] = await Promise.all(
                    [first, second, plain].map((client) => client.nextFrame()),
                );
                expectDraftAtFrame(a!, 0, 1_000_000);
                expectDraftAtFrame(b!, 2, 2_000_000);
                expect(c).not.toHaveProperty("draft");
                expect({ ...a, draft: undefined }).toEqual({
                    ...c,
                    draft: undefined,
                });
                expect({ ...b, draft: undefined }).toEqual({
                    ...c,
                    draft: undefined,
                });
                ...
                for (let pass = 1; pass <= 8; pass += 1) {
                    ...
                    running.sample();
                    const [one, two, none] = await Promise.all(
                        [first, second, plain].map((client) => client.next()),
                    );
                    kinds.push(one!.t);
                    ...
                    if (
                        ...
                        expect(
                            (one as QuotesMessage).changes.every(
    ...
    it("keeps draft input out of the game log and quotes a simple bell fixture without changing cash", () => {
        ...
        const connection: Connection = {
            ...
            sessionId: null,
            playerId: null,
            messages: createWindowCounter(20, 10_000),
        };
        ...
        market.days[0]!.paths = market.days[0]!.paths.map(() => [
            100,
            ...Array.from({ length: 500 }, () => 110),
        ]);
