...
export default function App() {
  ...
  function endGame() {
    ...
    const snapshot = {
      frame,
      pending: connection.pending.get().length > 0,
      stale: lineStateOf(connection.state.get().phase) !== 'live',
    };
    trackEvent('game_ended_early', {
      day: frame.clock.day,
      purchases: frame.positions.length,
      pace: frame.clock.pace ?? 1,
    });
    leaveGame();
    setExit(snapshot);
  }
  ...
  if (
    ...
    new URLSearchParams(window.location.search).get('landing') !== 'original'
  ) {
    return (
      <>
        <RecoveryNotice />
  ...
  return (
    <div
      className={`app-shell flex min-h-dvh flex-col ${phase !== null && phase !== 'lobby' && phase !== 'final' ? 'app-shell-desk' : ''}`}
    >
      <TopBar
        {...(phase !== null && phase !== 'lobby' && phase !== 'final' ? { onEnd: endGame } : {})}
      />
      <RecoveryNotice />
      {route === null || phase === 'lobby' ? (
