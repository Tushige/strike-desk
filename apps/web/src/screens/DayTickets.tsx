...
function DayTicket({
  ...
  useEffect(() => {
    ...
    const motion = stamp.current.animate(
      [
        { transform: 'scale(1.55) rotate(-12deg)', opacity: 0.3 },
  ...
  const description =
    change === undefined
      ...
      : change === 0
        ? 'no change'
        : `${outcome}, ${signedMoney(change)}`;
  const label = `Day ${String(day)}, ${description}`;
  return (
    <li
      className="day-ticket"
      data-state={state}
      ...
      title={label}
    >
      <span className="day-ticket-label" aria-hidden="true">
        DAY
      </span>
      <span className="day-ticket-number" aria-hidden="true">
        {day}
      </span>
...
export function DayTickets({ frame }: { frame: Frame | null }) {
  ...
  return (
    <ol className="day-tickets" aria-label="Five trading days">
      {Array.from({ length: DAYS }, (_, index) => {
        const day = index + 1;
        return (
          <DayTicket
            key={`${frame?.session ?? 'connecting'}:${String(day)}`}
            day={day}
