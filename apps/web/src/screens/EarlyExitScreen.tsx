      ...
      <main className="end-screen">
        <div className="summary-scene summary-journey">
          <div className="summary-heading">
            <div className="summary-verdict summary-verdict-with-pup">
              <div className="summary-verdict-copy">
                <h1 ref={heading} tabIndex={-1}>
                  You rang your own bell.
                </h1>
                <p>
                  {frame.positions.length === 0
                    ? 'A little window-shopping, then a well-earned snack break. RoboPup approves.'
                    : 'The market can wait. RoboPup is closing your tab and keeping the receipt.'}{' '}
                  You chose to leave on day {frame.clock.day}, with {frame.days.length} of 5 days
                  complete.
                </p>
              </div>
            ...
            <div className="summary-balance">
              ...
              <p className="summary-caption">
                Cash {money(frame.account.cashCents)}
                {openTickets.length > 0 && (
                  <>
                    {' '}
                    · Open positions{' '}
                    {money(openTickets.reduce((sum, position) => sum + position.valueCents, 0))}
                  </>
                )}
              </p>
            </div>
          ...
          <div className="summary-journey-body">
            {(exit.pending || exit.stale) && (
              <p className="text-sm text-sun">
                {exit.pending
                  ? 'An order was still being checked when you left. It will not be retried; this snapshot may not include its result.'
                  : 'The connection was interrupted. These are the last values received, which may be out of date.'}
              </p>
            )}
            {!completeToday && (
              <section
                aria-label="Unfinished day"
                className="mb-6 rounded-xl bg-raised p-4 text-sm leading-relaxed"
              >
                <h2 className="m-0 mb-2 font-semibold">
                  Day {frame.clock.day} · Clocked out early
                </h2>
                <p className="m-0 text-muted">
                  {openTickets.length > 0 ? (
                    <>
                      {openTickets.length}{' '}
                      {openTickets.length === 1 ? 'position was' : 'positions were'} still open.
                      Their last values are included above, but ending the game did not cash them
                      out or settle them.
                    </>
                  ) : currentTrades.length > 0 ? (
                    <>
                      You cashed out today for{' '}
                      {money(
                        currentTrades.reduce(
                          (sum, position) =>
                            sum + (position.exit?.proceedsCents ?? position.valueCents),
                          0,
                        ),
                      )}
                      , a{' '}
                      {signedMoney(
                        currentTrades.reduce((sum, position) => sum + position.profitCents, 0),
                      )}{' '}
                      result. The day was not completed.
                    </>
                  ) : (
                    'No ticket bought today. This unfinished day is not counted as a completed day.'
                  )}
                </p>
              </section>
            )}
            {frame.days.length > 0 ? (
              <>
                ...
                <div
                  className="summary-days summary-day-strip"
                  role="group"
                  aria-label="Review completed days"
                >
                  {frame.days.map((day) => (
                    <button
                      key={day.day}
                    ...
                    >
                      ...
                      <span className={day.changeCents < 0 ? 'summary-loss' : 'summary-profit'}>
                        {signedMoney(day.changeCents)}
                      </span>
                    </button>
                  ))}
                </div>
              ...
              <p className="mb-6 text-sm text-muted">
                No closing bells yet. A fresh five-day run is waiting whenever you are.
              </p>
            )}
          </div>
          <div className="summary-footer">
            ...
            <p>
              This run is over for you. A new game starts with fresh pretend money and a new market.
            </p>
          </div>
        </div>
      </main>
