...
export function RecoveryNotice() {
  ...
  if (value === null || value.intent.command.commandId !== restoredIntent?.command.commandId)
    return null;
  const outcome = value.outcome;
  const text =
    outcome === null
      ? 'Checking your last request…'
      : outcome.outcome === 'lost'
        ...
        : outcome.outcome === 'accepted'
          ? 'Your last request was confirmed by the server.'
          : outcome.receipt.reason === undefined
            ? 'Your last request was declined.'
            : REJECT_WORDS[outcome.receipt.reason];
  const checking =
    pending.find((p) => p.command.commandId === value.intent.command.commandId)?.status ===
    'checking';
  return (
    <aside role="status" className="border-b border-line px-4 py-2 text-sm text-sun">
      {text}{' '}
      {outcome === null && checking && (
        <button
          type="button"
          className="ml-3 underline"
          disabled={line.phase !== 'live'}
          onClick={() => {
            connection.resend(value.intent.command.commandId);
          }}
        >
          Retry safely
        </button>
