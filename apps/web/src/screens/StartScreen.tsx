...
export function StartScreen({ connected }: { connected: boolean }) {
  ...
  const { serverFull, gameGone } = useConnectionState();
  ...
  return (
    <main className="mx-auto flex w-full max-w-[1440px] grow flex-col justify-center gap-8 px-5 py-10 sm:px-12 lg:px-24">
      <div className="flex flex-col gap-4">
        <p className="m-0 text-[15px] font-semibold text-sun">{startWords.kicker}</p>
        <h1 className="m-0 font-display text-4xl leading-[1.08] font-extrabold tracking-tight sm:text-5xl lg:text-[64px]">
      ...
      <div className="flex flex-wrap items-center gap-x-8 gap-y-4">
        <PrimaryButton className="h-16 px-10 text-lg" disabled={!connected || opening} onClick={() => void open()}>
          {opening ? startWords.opening : connected ? startWords.action : startWords.connecting}
        </PrimaryButton>
