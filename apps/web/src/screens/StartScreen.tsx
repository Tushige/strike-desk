...
export function StartScreen({ connected }: { connected: boolean }) {
  const { pace, setPace, opening, live, open, serverFull, gameGone, failed, status } =
    useStartGame(connected);
  ...
  return (
    <main className="mx-auto flex w-full max-w-[1440px] grow flex-col justify-center gap-8 px-5 py-10 short:gap-5 short:py-5 sm:px-12 lg:px-24">
      <div className="flex flex-col gap-4">
        ...
        <p className="m-0 max-w-[52rem] text-lg leading-normal text-muted sm:text-xl">
          {startWords.lede}
        </p>
      </div>
      ...
      <ol className="m-0 grid list-none grid-cols-1 gap-5 p-0 md:grid-cols-3">
        {startWords.steps.map((step, i) => (
          <li
            key={step.title}
            className="flex flex-col gap-2.5 rounded-3xl border border-line bg-panel p-6"
          >
            ...
            <span className="leading-normal text-muted">
              {i === 1 ? (
                <>
                  Think the price will climb? Buy an <strong className="text-mint">UP</strong>{' '}
                  ticket. Think it will drop? Buy a <strong className="text-coral">DOWN</strong>{' '}
                  ticket. Then pick a target price.
                </>
              ) : (
                step.body
              )}
      ...
      <div className="flex flex-wrap items-center gap-x-8 gap-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div
            className="flex gap-1 rounded-2xl border border-line bg-panel p-1"
            role="group"
            ...
            {PACES.map((option) => (
              <ChoiceButton
                ...
                className={cx(
                  ...
                  option === pace ? 'bg-sun text-ink' : 'text-cloud',
                )}
              >
                {PACE_WORDS[option]}
              </ChoiceButton>
        ...
        {serverFull && (
          <p className="m-0 text-sm font-semibold text-coral" role="status">
