...
class GridBoundary extends Component<{ children: ReactNode; retry: () => void }, { failed: boolean }> {
  override state = { failed: false };
  static getDerivedStateFromError() { return { failed: true }; }
  override render() {
    if (this.state.failed) return <div role="alert" className="p-4 text-sm">The comparison board could not load. Your game is still available.
      <button type="button" className="ml-2 underline" onClick={this.props.retry}>Retry board</button></div>;
