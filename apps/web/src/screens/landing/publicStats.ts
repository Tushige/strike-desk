...
export function parseLandingStats(value: unknown): LandingStats {
  ...
  const unsigned = (input: unknown): input is string => typeof input === 'string' && /^\d{1,40}$/.test(input);
  const signed = (input: unknown): input is string => typeof input === 'string' && /^-?\d{1,40}$/.test(input);
  if (row.currency !== 'pretend-USD' || !unsigned(row.completedGames) || !unsigned(row.pretendProfitsEarnedCents) ||
      !(row.bestNetProfitCents === null || signed(row.bestNetProfitCents))) throw new Error('Invalid statistics');
  return { completedGames: row.completedGames, pretendProfitsEarnedCents: row.pretendProfitsEarnedCents,
    bestNetProfitCents: row.bestNetProfitCents };
}

const grouped = new Intl.NumberFormat('en-US');
const UNITS = [[1_000_000_000_000_000n, 'Q'], [1_000_000_000_000n, 'T'], [1_000_000_000n, 'B'],
  [1_000_000n, 'M'], [1_000n, 'K']] as const;
...
export function statNumber(value: string, money = false): { compact: string; exact: string } {
  ...
  for (const [unit, suffix] of UNITS) {
    ...
    const fraction = (rounded % decimals).toString().padStart(decimals === 10n ? 1 : 2, '0').replace(/0+$/, '');
    return { compact: `${prefix}${rounded / decimals}${fraction ? `.${fraction}` : ''}${suffix}`, exact };
  }
  return { compact: `${prefix}${grouped.format(absolute / scale)}${cents ? `.${cents.toString().padStart(2, '0')}` : ''}`, exact };
}
