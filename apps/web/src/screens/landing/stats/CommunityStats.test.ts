...
const data = {
  completedGames: '12',
  pretendProfitsEarnedCents: '140000000',
  bestNetProfitCents: '140000000',
};
...
vi.mock('gsap', () => ({
  gsap: {
    timeline: (options: unknown) => {
      ...
      return { fromTo: motion.fromTo, play: motion.play, kill: motion.kill };
    },
    set: motion.set,
    context: (work: () => void) => {
      work();
...
it('starts all three ticket numbers from one section visibility trigger', () => {
  ...
  expect(
    motion.options.mock.calls.every(([options]) => (options as { delay: number }).delay === 0),
  ).toBe(true);
});
...
it('rolls only digits on first visibility, without replaying or rounding fresh totals', () => {
  ...
  for (const [node, from, to] of motion.fromTo.mock.calls) {
    ...
    expect(to).toEqual({
      y: -Number((node as HTMLElement).dataset.digit) * 20,
      yPercent: 0,
      duration: 1.5,
      ease: 'power2.out',
    });
  ...
  expect(observed[0]!.classList.contains('community-metrics')).toBe(true);
  act(() => {
    callbacks[0]!();
  });
  expect(motion.play).toHaveBeenCalledTimes(3);
  ...
  view.rerender(
    createElement(CommunityStats, {
      variant: 'ticket',
      state: { status: 'ready', data: { ...data, completedGames: '13' } },
      onRetry: () => {},
    }),
  );
  expect(motion.fromTo).toHaveBeenCalledTimes(entrances);
  expect(
    view.container.querySelector('.community-games [role="img"]')?.getAttribute('aria-label'),
  ).toBe('13');
  expect(motion.set).toHaveBeenCalled();
  view.unmount();
  expect(motion.kill).toHaveBeenCalledTimes(motion.options.mock.calls.length);
});
...
it('keeps digit 5 inside its window and leaves no empty slot before a compact suffix', () => {
  ...
  const five = motion.fromTo.mock.calls.find(
    ([node]) => (node as HTMLElement).dataset.digit === '5',
  );
  ...
  expect(
    suffix.previousElementSibling?.querySelector('[data-digit]')?.getAttribute('data-digit'),
  ).toBe('2');
  expect(
    motion.set.mock.calls.every(([, vars]) => (vars as { yPercent: number }).yPercent === 0),
  ).toBe(true);
});
...
it('shows an honest error then retries using real API values, without substituting sample figures', async () => {
  const fetchMock = vi
    ...
    .mockResolvedValueOnce({ ok: true, json: () => ({ ...data, currency: 'pretend-USD' }) });
