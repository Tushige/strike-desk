...
export function RoboPup({ pose, active = true }: { pose: Pose; active?: boolean }) {
    ...
    if (pose === 'stamp') {
      motions.push(
        art.animate(
          [
            { transform: 'none' },
            { transform: 'translateY(4px) rotate(-5deg)', offset: 0.38 },
            { transform: 'none' },
          ],
      ...
      if (stamp)
        motions.push(
          stamp.animate(
            [
              { opacity: 0, transform: 'rotate(-9deg) scale(1.3)' },
              { opacity: 1, transform: 'rotate(-9deg) scale(1)' },
            ],
            { duration: 240, delay: 180, fill: 'backwards', easing: ease },
          ),
        );
    } else if (pose === 'bell') {
      motions.push(
      ...
      root.current?.querySelectorAll('.robopup-ring').forEach((ring, index) => {
        motions.push(
          ring.animate(
            [
              { opacity: 0.7, transform: 'scale(.5)' },
              { opacity: 0, transform: 'scale(2.3)' },
            ],
            { duration: 550, delay: 150 + index * 90, easing: 'ease-out' },
          ),
        );
      });
    } else {
  ...
  return (
    <span ref={root} className={`robopup robopup-${pose}`} aria-hidden="true">
      <span className="robopup-art">
        <img
          src={poses}
          alt=""
          draggable={false}
