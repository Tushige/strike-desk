...
export function DayReview({ frame, day }: { frame: Frame; day: number }) {
  ...
  const review = result?.reviews === undefined
    ? position === positions[0] ? result?.review : undefined
  ...
  return <section aria-label={`Day ${String(day)} review`} className="border-t border-line pt-4 text-sm" aria-live="polite">
    <h3 className="m-0 mb-3 font-bold">Day {day} · {positions.length > 1 ? `${String(positions.length)} purchases` : position === undefined ? 'No trade' : `${frame.companies[position.companyId]?.name ?? ''} ${position.side === 'up' ? 'UP / call' : 'DOWN / put'}`}</h3>
    {positions.length > 1 && position && <><PositionList frame={frame} positions={positions} selectedId={position.id} onSelect={setSelectedId} /><h4 className="my-4 font-semibold">{frame.companies[position.companyId]?.name} {position.side === 'up' ? 'UP / call' : 'DOWN / put'}</h4></>}
