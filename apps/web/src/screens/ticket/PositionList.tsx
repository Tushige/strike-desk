...
export function PositionList({
  ...
  return (
    <div className="purchase-list" role="group" aria-label="Purchases">
      {positions.map((position, index) => {
        const company = frame.companies[position.companyId];
        const status =
          position.status === 'open'
            ? 'Open'
            : position.status === 'cashedOut'
              ? 'Cashed out'
              : 'Settled';
        return (
          <button
          ...
          >
            <strong>
              {company?.ticker}{' '}
              <span className={position.side === 'up' ? 'text-mint' : 'text-coral'}>
                {position.side.toUpperCase()}
              </span>{' '}
              · {price(position.targetCents)}
            </strong>
            <strong className="purchase-number">
              {money(position.exit?.proceedsCents ?? position.valueCents)}
            </strong>
            <span>
              Purchase {index + 1} · {status}
            </span>
            <span
              className={`purchase-number ${position.profitCents < 0 ? 'text-coral' : position.profitCents > 0 ? 'text-mint' : ''}`}
            >
