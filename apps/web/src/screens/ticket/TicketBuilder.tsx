...
import {
  ...
  quoteEchoes,
  stopAt,
} from '../../modules/order-ticket/index';
...
const QUIET_BLOCKERS: readonly (BuyBlocker | null)[] = [
  null,
  'notDraft',
  'noContract',
  'noSpend',
  ...
  return (
    ...
    >
      ...
      <span className="text-xs font-medium">
        {isUp ? 'Pays above target' : 'Pays below target'}
      </span>
    </ChoiceButton>
  );
}
...
function summaryOf(machine: TicketMachine, pick: Pick) {
  ...
  if (pick.side === null && pick.contractId === null)
    return 'Pick UP or DOWN to start your ticket.';
  ...
  if (quote.quantity < 1)
    return 'That is not enough for even one ticket. Spend more or pick a cheaper target.';
  const { contract } = snapshot;
  const side = contract.side === 'up' ? 'UP' : 'DOWN';
  return (
    <span className="grid grid-cols-[minmax(0,1fr)_auto] gap-x-2">
      <span className="col-span-2 whitespace-nowrap tabular-nums">
        {count(quote.quantity)} {side} tickets
      </span>
      <span>Price limit</span>
      <span className="text-right whitespace-nowrap tabular-nums">
        {money(quote.limitPriceCents)} /ticket
      </span>
    </span>
  );
}
...
function WhatIf({ quote, ticker }: { quote: TicketQuote; ticker: string }) {
  ...
  const index =
    chosen !== null && chosen.contractId === quote.contractId
      ? Math.min(chosen.index, last)
      : breakEvenStopIndex(quote);
  ...
  return (
    <div
      ...
      data-outcome={stop.profitCents === 0 ? 'even' : ahead ? 'profit' : 'loss'}
    >
      <label htmlFor={id} className="text-[12px] text-muted">
        Scenario: what if, at the closing bell, {ticker} is at{' '}
        <span className="inline-block w-[8ch] text-right font-semibold text-cloud tabular-nums">
          {price(stop.atCents)}
        </span>
        ?
      </label>
      <div className="scenario-result">
        ...
        <strong className="tabular-nums">
          {stop.profitCents === 0 ? '$0' : signedMoney(stop.profitCents)}
        </strong>
      </div>
      <div className="scenario-track">
        {exactBreakEven && last > 0 && (
          <span
            className="scenario-break-even"
            style={{ left: `${String((breakEven / last) * 100)}%` }}
            aria-hidden="true"
          />
        )}
        <input
          ...
          onChange={(event) => {
            setChosen({ contractId: quote.contractId, index: Number(event.currentTarget.value) });
          }}
        />
      </div>
  ...
  return (
    <div className="ticket-builder flex min-h-full flex-col gap-3 short:gap-2.5">
      ...
      <div className="ticket-content">
        {frame.stress && (
          <p className="m-0 flex items-start gap-2 text-xs text-sun">
            <InfoIcon className="mt-0.5 size-4 shrink-0" />
            <span>
              Read-only workload. Latest complete quote scenario, refreshed about every 1.5 seconds.
              Table prices can move between scenarios.
            </span>
          </p>
        )}
        ...
        <div className="flex flex-col gap-2 short:gap-1.5">
          ...
          {targets.map((target) => {
            const selected =
              pick.side !== null && snapshot.contract?.contractId === target.contractId;
              ...
              >
                ...
                <span className="flex shrink-0 flex-col items-end gap-px whitespace-nowrap tabular-nums">
                  ...
                  <span className="text-xs text-muted">
                    {ticketPrice === undefined ? '' : `${money(ticketPrice)} /ticket`}
                  </span>
                </span>
              </ChoiceButton>
            );
          ...
          {pick.contractId !== null && pick.choice === null && snapshot.contract !== null && (
            <div className="grid min-h-[58px] grid-cols-2 items-center gap-3 rounded-lg border border-sun bg-raised px-3 py-2 text-left text-cloud short:min-h-[48px] short:py-1">
              <span className="flex flex-col gap-px">
                ...
                <span className="text-xs text-muted">
                  {snapshot.contract.ticker} {snapshot.contract.side === 'up' ? 'UP' : 'DOWN'}
                </span>
              </span>
              <span className="flex shrink-0 flex-col items-end gap-px whitespace-nowrap tabular-nums">
                <span className="text-[15px] font-bold">
                  {price(snapshot.contract.targetCents)}
                </span>
                <span className="text-xs text-muted">
                  {frame.quotes[snapshot.contract.contractId] === undefined
                    ? ''
                    : `${money(frame.quotes[snapshot.contract.contractId] ?? 0)} /ticket`}
                </span>
              </span>
            </div>
          )}
          {targets.length === 0 &&
            CHOICES.map((choice) => (
              <div
                key={choice}
                className="flex min-h-[58px] items-center rounded-lg border border-line px-3 text-sm text-muted short:min-h-[48px]"
              >
                {CHOICE_WORDS[choice]}
              </div>
            ))}
        ...
        <div className="flex flex-col gap-2 short:gap-1.5">
          <div className="flex items-baseline justify-between gap-2">
            ...
            <button
              type="button"
              className="text-xs text-muted underline underline-offset-4 hover:text-cloud"
              disabled={locked}
              aria-expanded={showCustom}
              aria-controls={`${customId}-field`}
              onClick={() => {
                setCustomDay(frame.clock.day);
              }}
            >
              Custom
            </button>
          ...
          {showCustom && (
            <div id={`${customId}-field`} className="flex flex-col gap-1">
              <label htmlFor={customId} className="text-xs text-muted">
              ...
              <input
                ...
                inputMode="numeric"
                autoComplete="off"
                className="w-full rounded-xl border border-line bg-well px-3 py-2 text-sm text-cloud"
                value={editing ?? (state.spendCents === null ? '' : String(state.spendCents / 100))}
                disabled={locked}
                aria-invalid={budgetError !== null}
                aria-describedby={budgetError === null ? undefined : `${customId}-error`}
                onChange={(event) => {
                  const text = event.currentTarget.value;
                  setCustom({ day: frame.clock.day, text });
                  handlers.onChooseSpend(parseBudget(text, cap).cents);
                }}
              />
            </div>
          )}
          {budgetError !== null && (
            <p id={`${customId}-error`} className="m-0 text-xs text-coral" role="status">
              {budgetError}
            </p>
          )}
        </div>
      ...
      <ActionDock>
        <div className="ticket-cost-summary flex flex-col gap-3 text-sm leading-snug short:text-[13px]">
          ...
          {quote !== null &&
            quote.quantity > 0 &&
            quote.whatIf.length > 0 &&
            snapshot.contract !== null && (
              <details className="scenario-disclosure text-xs text-muted">
                <summary className="cursor-pointer hover:text-cloud">
                  Explore a price scenario
                </summary>
                <div className="pt-2">
        ...
        <PrimaryButton
          className={cx(
            'h-[52px] short:h-11',
            quote !== null && quote.quantity > 0 && 'live-trade-action',
          )}
          disabled={blocker !== null || budgetError !== null}
          onClick={() => {
            handlers.onPress('buy');
          }}
        >
          {quote !== null && quote.quantity > 0 ? (
            <>
              <span>Buy for</span>
              <span>{money(quote.costCents)}</span>
            </>
          ) : (
            'Buy ticket'
          )}
        </PrimaryButton>
        {!QUIET_BLOCKERS.includes(blocker) &&
          blocker !== null &&
          BLOCKER_WORDS[blocker] !== null && (
