...
export function TicketPanel({
  ...
  const pending =
    usePendingCommands().some((item) => item.command.t === 'buy' || item.command.t === 'cashOut') ||
    machine.state.form === 'pending' ||
    machine.state.form === 'checking';
  ...
  }, [selectedPositionId, positions.length]);
  useEffect(() => {
    ...
    <div className="purchase-panel">
      <div className="purchase-allowance">
        <div className="purchase-title">
          ...
          <span>
            {positions.length} of {MAX_DAILY_PURCHASES} purchases
          </span>
        </div>
        <div className="purchase-marks" aria-hidden="true">
          {Array.from({ length: MAX_DAILY_PURCHASES }, (_, index) => (
            <span key={index} className={index < positions.length ? 'used' : ''} />
          ))}
        </div>
        <div className="purchase-budget">
          <span>Daily allowance left</span>
          <strong>{money(frame.account.capCents)}</strong>
        </div>
      </div>
      {ticket !== null && left > 0 && !frame.stress && (
        <button
          type="button"
          className="purchase-new"
          disabled={pending}
          onClick={() => {
      ...
      <div className="purchase-holdings">
        {positions.length > 0 ? (
          ...
          <p className="purchase-empty">Up to three purchases today. Start with one below.</p>
        )}
        {left === 0 && (
          <p className="purchase-empty">
            All three purchases used. You can still cash out each open position.
          </p>
        )}
      </div>
      <div className="purchase-detail" ref={detail}>
        ...
        {ticket?.status === 'open' && (
          <LiveTicket frame={frame} position={ticket} machine={machine} />
        )}
        {ticket !== null && ticket.status !== 'open' && (
          <CashedOut frame={frame} position={ticket} />
        )}
      </div>
    </div>
  );
