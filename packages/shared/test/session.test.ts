...
import { ME, TEST_IDENTITY, buyCommand, clockCommand, findContract, me, priceOf, start } from './helpers';

const T0 = 1_700_000_000_000;

function startedSession(pace: 1 | 3 | 7.5 = 1): Session {
  const handled = handleCommand(createSession('s1', TEST_IDENTITY), ME, start(pace), T0);
...
describe('session', () => {
  it('waits in the lobby at step 0 however much time passes', () => {
    ...
    const { frame, session: same } = frameFor(session, ME, T0 + 3_600_000, { history: true });
  ...
  it('makes a board of the wanted size', () => {
    ...
    expect(frameFor(startedSession(), ME, T0, { history: false }).frame.quotes).toHaveLength(252);
    expect(frameFor(handleCommand(session, ME, start(1), T0).session, ME, T0, { history: false }).frame.quotes).toHaveLength(2508);
  });
  ...
  it('keeps the first clock when start is sent again', () => {
    ...
    const handled = handleCommand(session, ME, start(7.5), T0 + 5_000);
  ...
  it('re-anchors on a jump so time continues from the jump target', () => {
    const session = startedSession(1);
    const handled = handleCommand(session, ME, clockCommand('openBell', 1), T0 + 2_000);
  ...
  it('does not re-anchor on a refused jump or a repeat', () => {
    const session = startedSession(1);
    const refused = handleCommand(session, ME, clockCommand('skipToBell', 1), T0 + 2_000);
    ...
    const first = handleCommand(session, ME, jump, T0 + 2_000);
    const again = handleCommand(first.session, ME, jump, T0 + 9_000);
  ...
  it('never steps back if the wall clock does', () => {
    const jumped = handleCommand(startedSession(1), ME, clockCommand('openBell', 1), T0 + 2_000).session;
    expect(sessionStep(jumped, T0)).toBe(300);
  });

  it('settles at the bell when a frame is asked for, and leaves its input alone', () => {
    const opened = handleCommand(startedSession(1), ME, clockCommand('openBell', 1), T0).session;
    ...
    const held = handleCommand(opened, ME, buy, T0 + 1_000).session;
    expect(me(held.game).positions[0]).toMatchObject({ entryStep: 305, entryPriceIndex: 5 });

    const before = frameFor(held, ME, T0 + 99_999, { history: false });
    expect(before.frame.step).toBe(799);
    expect(before.frame.positions[0]?.status).toBe('open');

    const atBell = frameFor(held, ME, T0 + 100_000, { history: false });
    ...
    expect(atBell.frame.rev).toBe(me(held.game).rev + 1);
    expect(atBell.frame.account.cashCents).toBe(me(held.game).cashCents + priceOf(market, 1, 500, contract) * (me(held.game).positions[0]?.quantity ?? NaN));
    expect(me(atBell.session.game).dayEndCents).toEqual([atBell.frame.account.cashCents]);
    expect(me(held.game).dayEndCents).toEqual([]);

    const later = frameFor(atBell.session, ME, T0 + 100_000, { history: false });
  ...
  it('reaches the final screen and stays there', () => {
    const { frame } = frameFor(startedSession(7.5), ME, T0 + 10_000_000, { history: false });
